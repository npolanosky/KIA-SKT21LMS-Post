...
// user-defined properties //ANCHOR Properties
properties = {
 ...
 useG28Zhome: false, // use G28 to move Z to its home position
  ...
  partingToolKerf: 0.118, //width of parting tool, used for bar puller calculations in inches
  useBarPuller: true, //if true will call the subprogram for barpull and use M99, else will use M30
  barPullerVariable: 550, //Macro variable number to use for bar puller subprogram
  barPullerRadius: 1.5, //Radius of bar puller arms in inches
  barPullerProgram: 8888 //Subprogram number to use for bar puller program
};
...
propertyDefinitions = {
 ...
 cutoffConfirmation: {title:"Use G350 parting confirmation", description:"Use G350 after cutoff for parting confirmation.", type:"boolean"},
  ...
  partingToolKerf: {title:"Parting Tool Kerf in Inches", description:"Set this to the width of the parting blade (inches) being used- for bar pull calculation", type:"number"},
  useBarPuller: {title:"Use Bar Puller", description:"Enable to use the bar puller subprogram and M99 instead of M30", type:"boolean"},
  barPullerVariable: {title:"Bar Puller Variable", description:"Variable number to use for bar puller position", type:"number"},
  barPullerRadius: {title:"Bar Puller Radius", description:"Radius of Bar Puller arms in inches", type:"number"},
  barPullerProgram: {title:"Bar Puller Program", description:"Program number to call bar puller subprogram", type:"number"}
};
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    gMotionModal.reset();
      ...
      writeBlock("G4X0.25 (TURRET DELAY)");
    if (tool.comment) {
...
function stopSpindle() {
  ...
  if (machineState.spindlesAreSynchronized) {
    ...
    writeBlock(
     ...
     // writeBlock("(STOP SPINDLE)"),
      //spOutput.format(getCode("SELECT_SPINDLE", activeSpindle)),
     // writeBlock("(SELECT SPINDLE)") 
    );
  }
  lastSpindleSpeed = 0;
